...
class CdrService extends \Core\Defaults\DefaultModel{
    ...
    public function GraficoHora(string $hora){
        try{
            $query = "  SELECT DATE_FORMAT( calldate, '%H:00') AS hora_truncada, COUNT(*) AS registros, status
                        ...
                        AND src NOT IN (SELECT id_sip FROM {$_ENV['DBNAME']}.sip)
    ...
    public function GetLinkedQueue(){
        try{
            $query      = "SELECT * FROM {$_ENV['DBNAME']}.queue where queue_code not in (99,9900)";
            $exts       = $this->executeQuery($query);
            $extensions = [];
            $re         = '/(\d+)@/m';

            foreach($exts as $key => $ext){
                $qry                        = "SELECT member FROM {$_ENV['DBNAME']}.queue_members WHERE id_queue = {$ext['id_queue']}";
    ...
    public function GetDevices(){
        try{
            $query = "SELECT id_sip, concat(id_sip, ' - ', callerId) as text  FROM {$_ENV['DBNAME']}.sip";
    ...
    public function CallReports($where){
        try{
            $query = "SELECT 
                        ...
                        (SELECT d.callerId FROM {$_ENV['DBNAME']}.sip as d WHERE d.id_sip = c.dstchannel) as dst_name,
                        (SELECT d.callerId FROM {$_ENV['DBNAME']}.sip as d WHERE d.id_sip = c.src) as src_name
                    FROM
