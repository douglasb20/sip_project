...
class CdrService extends \Core\Defaults\DefaultModel{
    ...
    public function GraficoHora(string $hora){
        try{
            $query = "  SELECT DATE_FORMAT( calldate, '%H:00') AS hora_truncada, COUNT(*) AS registros, disposition
                        ...
                        WHERE cdr.calldate BETWEEN '{$this->data} {$hora}:00:00' and '{$this->data} {$hora}:59:59' AND disposition IN ('ANSWERED', 'NO ANSWER', 'CONGESTION')
    ...
    public function AgrupaHora(){
        try{
            $query = "  SELECT DATE_FORMAT( calldate, '%H') AS hora_truncada
                        FROM asteriskcdrdb.cdr
                        WHERE cdr.calldate BETWEEN '{$this->data} 00:00:00' and '{$this->data} 23:59:59' AND disposition IN ('ANSWERED', 'NO ANSWER', 'CONGESTION')
    ...
    public function GeraDadosGrafico(){
        try{
